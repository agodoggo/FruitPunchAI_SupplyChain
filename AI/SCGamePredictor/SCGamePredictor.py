...
class SCGameRecommender(nn.Module):
    
    # Initializes the Supply Chain Game recommender agent from the specified neural network_file.
    def __init__(self, network_file):
        super(SCGameRecommender, self).__init__()
        self.linear1 = nn.Linear(18, 180)
        self.linear2 = nn.Linear(180, 130)
        self.linear3 = nn.Linear(130, 100)
        self.linear4 = nn.Linear(100, 100)
        self.linear5 = nn.Linear(100, 100)
        self.linear6 = nn.Linear(100, 108)

        model = torch.jit.load(network_file)        
    ...
    def forward(self, x):
        return self.linear6(F.relu(self.linear5(F.relu(self.linear4(F.relu(self.linear3(F.relu(self.linear2(F.relu(self.linear1(x)))))))))))
    ...
    def get_best_action(self, state, action_type):
        action_scores = self(torch.FloatTensor(state)).tolist()
